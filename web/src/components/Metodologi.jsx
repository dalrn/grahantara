...
export default function Metodologi({ onKembali, versi }) {
  ...
  const formatBilangan = (n) => (typeof n === "number" ? n.toLocaleString("id-ID") : "—");
  const formatPecahan = (n) => (typeof n === "number" ? `${n.toLocaleString("id-ID", { maximumFractionDigits: 1 })}%` : "—");
  ...
  const dimKosong = DIMENSI.filter((d) => kosongPerDimensi[d] > 0)
    .map((d) => `${KELOMPOK_INDIKATOR.find((x) => x.dimensi === d)?.label ?? d} (${formatBilangan(kosongPerDimensi[d])})`)
    .join(", ");
  ...
  if (data && jumlah) {
    for (const k of URUTAN_INDIKATOR) {
      ...
      for (const f of data.features) {
        const ik = f.properties.indikator?.[k];
        if (ik && typeof ik.nilai === "number" && typeof ik.persentil === "number") {
          xs.push(ik.nilai);
  ...
  for (const f of kosData) {
    ...
    sebarSumber[s == null || s === "" ? "tanpa label" : s] = (sebarSumber[s == null || s === "" ? "tanpa label" : s] ?? 0) + 1;
    const p = f.properties.presisi_koordinat;
    sebarPresisi[p == null ? "(tanpa presisi)" : p] = (sebarPresisi[p == null ? "(tanpa presisi)" : p] ?? 0) + 1;
    if (f.properties.harga_median == null) kosTanpaHarga++;
  }
  const ARTIKEL_PRESISI = {
    ruas_terkait: "koordinat diambil dari ruas jalan terkait, bukan titik bangunannya",
    pusat_kawasan: "koordinat memakai titik pusat kawasan",
    manual: "koordinat diisi manual oleh tim",
    nama_kos: "koordinat dipulihkan dari nama kos di catatan survei, bukan direkam saat survei",
  };
  ...
  if (data) {
    for (const k of URUTAN_INDIKATOR) {
      ...
      for (const label of iks.split(", ")) {
        const kunci = Object.entries(LABEL_SUMBER).find(([, l]) => l === label)?.[0] ?? label;
    ...
    <div className="flex h-full w-full flex-col overflow-y-auto bg-slate-950">
      <div className="mx-auto w-full max-w-[760px] px-4 py-6">
        ...
        <p className="mt-1 text-sm text-slate-300">
          {jumlah !== null ? `${formatBilangan(jumlah)} heksagon` : "—"} H3 resolusi 9,
          sabuk kampus Sleman, DIY. Sebaran skor pada data ini: 14,4–87,5.
        </p>
...
skor = 100 x PRODUK atas d di D dari (subskor_d/100 + 0,01)^(bobot_d / W)`}
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-300">
          <li>
            <b>Kenapa rata-rata geometrik, bukan aritmetik.</b> Bila satu dimensi mendekati nol,
            skor total ikut jatuh. Kos dengan warung melimpah tapi tanpa akses transit tetap
            salah pilihan bagi mahasiswa tanpa kendaraan.
          </li>
          <li>
            <b>Fungsi epsilon 0,01.</b> Mencegah satu dimensi nol membuat seluruh skor nol, dan
            menjaga fungsi tetap terdefinisi.
          </li>
          <li>
            <b>Konsekuensi epsilon.</b> Skor terendah yang mungkin adalah 1, bukan 0. Rumus juga
            bisa melebihi 100 bila seluruh subskor 100, karena itu hasil dipotong di 100.
          </li>
          {jumlah !== null && (
            <li>
              <b>Eksklusi dimensi kosong.</b> Dimensi yang seluruh indikatornya tidak tersedia
              ditulis subskor 0 di berkas data karena skema mewajibkan angka, TETAPI dikeluarkan
              dari perhitungan dan bobotnya dibagi ulang ke dimensi yang tersisa. Nol di situ
              berarti tidak ada data, bukan nilai nol. Pada data ini{" "}
              {formatBilangan(denganKosong)} dari {formatBilangan(jumlah)} heksagon memiliki
              dimensi kosong{dimKosong ? ` — ${dimKosong}` : ""}.
            </li>
          )}
          {denganKosong > 0 && (
            <li>
              <b>Konsekuensi bagi pengguna.</b> Pada heksagon itu skor dibentuk dari tiga
              dimensi yang tersisa, dan menggeser slider Keterjangkauan tidak mengubah skornya.
            </li>
          )}
        </ul>
            ...
            <tbody>
              {KELOMPOK_INDIKATOR.map((k) => (
                <tr key={k.dimensi} className="border-b border-slate-800">
                  ...
                  <td className="py-1">
                    {(((bobotMeta ?? BOBOT_DEFAULT)[k.dimensi] ?? 0) * 100).toLocaleString("id-ID")}%
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <h2 className="mt-6 text-xl font-semibold text-white">Cakupan data</h2>
        <p className="mt-1 text-sm text-slate-300">
          Jumlah heksagon yang punya nilai untuk tiap indikator. Cakupan yang jujur lebih
          bernilai daripada tabel yang terlihat penuh.
        </p>
        <div className="mt-2 overflow-x-auto">
          <table className="w-full text-sm text-slate-300">
            <thead>
                ...
                return (
                  <tr key={k} className="border-b border-slate-800">
                    <td className="py-1 pr-2">
                      {NAMA_INDIKATOR[k] ?? k}
                      <span className="ml-1 text-[10px] text-slate-500">{ik.dimensi}</span>
                    </td>
                    <td className="py-1 pr-2">{ik.satuan ?? "—"}</td>
                    <td className="py-1 pr-2">{ik.sumber || "—"}</td>
                    <td className="py-1 pr-2 text-right">{formatBilangan(ik.punya ?? 0)}</td>
                    <td className="py-1 text-right">
                      {cakupan === 0 ? (
                        <span className="rounded bg-red-900/70 px-1.5 py-0.5 text-[10px] font-semibold text-red-100">
                          0% — tanpa data
                        </span>
                      ) : (
        ...
        <p className="mt-2 text-xs text-slate-400">
          Indikator tanpa data dikeluarkan dari subskor dimensinya, dan dimensi yang seluruh
          indikatornya kosong dikeluarkan dari skor total.
        </p>

        <h2 className="mt-6 text-xl font-semibold text-white">Polaritas indikator</h2>
        <p className="mt-1 text-sm text-slate-300">
          Sebagian indikator berpolaritas terbalik: nilai lebih kecil berarti kondisi lebih
          baik (misalnya jarak ke halte, jarak ke stasiun, dan harga sewa). Pada data ini,
          dihitung dari arah nilai terhadap persentilnya:
        </p>
        ...
        <p className="mt-2 text-sm text-slate-300">
          Perhitungan skor dan perbandingan antar kawasan memakai persentil, bukan nilai
          mentah, sehingga polaritas sudah tertangani tanpa aturan khusus per indikator.
        </p>

        <h2 className="mt-6 text-xl font-semibold text-white">Survei lapangan</h2>
        {kosTotal ? (
          <>
            <p className="mt-1 text-sm text-slate-300">
              {formatBilangan(kosTotal)} titik kos disurvei tim di lapangan
              {kosTanpaHarga ? `; ${kosTanpaHarga} di antaranya tanpa harga tercatat.` : "."}
            </p>
            <h3 className="mt-3 text-sm font-semibold text-emerald-400">Cara harga diperoleh</h3>
            <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-slate-300">
            ...
            <h3 className="mt-3 text-sm font-semibold text-emerald-400">Presisi koordinat</h3>
            <ul className="mt-1 list-disc space-y-1 pl-5 text-sm text-slate-300">
              ...
              {Object.entries(LABEL_SUMBER).map(([kunci, label]) => (
                <tr key={kunci} className="border-b border-slate-800">
                  ...
                  <td className="py-1 pr-3">
                    {indikatorPerSumber[kunci]?.length ? indikatorPerSumber[kunci].join(", ") : "—"}
                  </td>
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-300">
          {jumlah !== null && (
            <li>
              ...
              {formatBilangan(jumlah - (indikator.A1_harga_kos?.punya ?? 0))} dari{" "}
              {formatBilangan(jumlah)} heksagon karena harga sewa hanya terdata di kawasan yang
              disurvei ({formatBilangan(indikator.A1_harga_kos?.punya ?? 0)} heksagon).
            </li>
          )}
          <li>Harga makan dan keramaian kawasan belum tersedia sama sekali.</li>
          <li>
            Harga sewa berasal dari {kosTotal ? formatBilangan(kosTotal) : "31"} titik survei —
            sebagian dari media sosial dan spanduk, bukan seluruhnya wawancara langsung.
          </li>
          <li>
            Sebagian koordinat kos adalah perkiraan tingkat ruas jalan, bukan titik bangunan
            (ruas_terkait {formatBilangan(sebarPresisi.ruas_terkait ?? 0)}, nama_kos{" "}
